...
if (mvccIndex !== -1) {
  ...
  if (specStart !== -1) {
    ...
    // Find the end by looking for the closing brace of the specs object
    const specEnd = content.indexOf("\n    }\n  }", htmlStart);
    // The htmlEnd is the last quote before specEnd
    const htmlEnd = content.lastIndexOf("'", specEnd);
    ...
    if (tablesIndex !== -1) {
      ...
      console.log("Could not find splitKey. htmlStr length is " + htmlStr.length);
    }
  }
}
