...
let html = `
  ...
  <!-- Table 1 -->
  <div>
    <p class="font-semibold text-gray-700 mb-4 mt-12">Cross Section & Construction Details</p>
...
// The regex matches `application: [...]`
const regex = /application:\s*\[[\s\S]*?\]/;

// Make sure we only replace it for MVCC.
// We can find the MVCC block first.
const mvccStart = productData.indexOf("name: 'Medium Voltage Covered Conductor (MVCC)',");
const mvccEnd = productData.indexOf("name: 'Solar Cables',", mvccStart);

let mvccBlock = productData.substring(mvccStart, mvccEnd);
mvccBlock = mvccBlock.replace(regex, `specificationHtml: '${html.replace(/'/g, "\\'")}'`);

const newProductData = productData.substring(0, mvccStart) + mvccBlock + productData.substring(mvccEnd);

fs.writeFileSync('src/data/productData.js', newProductData);
console.log('Replaced application array with specificationHtml for MVCC');
